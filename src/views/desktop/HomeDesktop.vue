  ...
  <div class="home">
    ...
    <section class="feature-section">
      <div class="fs-head rv">
        <div class="sh-eyebrow serif">— 为什么选择</div>
...
/* —— 顶栏 —— */
.mast { border-bottom: 1px solid var(--rf-line-soft); background: color-mix(in srgb, var(--rf-paper) 88%, #fff); backdrop-filter: blur(8px); position: sticky; top: 0; z-index: 50; }
...
.wm-mark { width: 34px; height: 34px; display: grid; place-items: center; background: var(--rf-ink); color: var(--rf-paper); font-family: var(--rf-font-serif); font-size: 17px; border-radius: 9px; }
...
.t-accent { color: var(--rf-accent); }
.hero-sub { color: var(--rf-ink-2); font-size: 16px; line-height: 1.8; max-width: 480px; margin: 0 0 32px; }
.hero-cta { display: flex; gap: 14px; flex-wrap: wrap; }
.btn-ink { background: var(--rf-ink); border: 1px solid var(--rf-ink); color: #fff; font-weight: 600; padding: 0 26px; }
.btn-ink:hover { background: var(--rf-accent); border-color: var(--rf-accent); color: #fff; }
.btn-line { background: transparent; border: 1px solid var(--rf-line); color: var(--rf-ink); font-weight: 500; }
.btn-line:hover { border-color: var(--rf-accent); color: var(--rf-accent); }
.hero-saved { margin-top: 26px; display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
.hs-label { font-size: 13px; color: var(--rf-faint); }
.hs-chip { border: 1px solid var(--rf-line); background: var(--rf-card); color: var(--rf-ink-2); font-size: 12px; padding: 3px 10px; border-radius: 999px; cursor: pointer; }
.hs-chip:hover { border-color: var(--rf-accent); color: var(--rf-accent); }
...
.doc-front { transform: rotate(1deg); box-shadow: var(--rf-shadow-md); padding: 26px 24px; display: flex; flex-direction: column; gap: 14px; }
...
.prof-card {
  ...
  padding: 26px 22px 20px; cursor: pointer; transition: transform .25s cubic-bezier(.2,.7,.2,1), box-shadow .25s, border-color .25s;
}
.prof-card:hover { transform: translateY(-4px); box-shadow: var(--rf-shadow-md); border-color: #d8cdbE; }
.prof-card:hover .pc-arrow { transform: translateX(4px); color: var(--rf-accent); }
.pc-num { position: absolute; top: 18px; right: 20px; font-size: 22px; color: var(--rf-line); font-weight: 600; }
.pc-top { display: flex; justify-content: space-between; align-items: center; margin-bottom: 18px; }
.pc-icon { font-size: 30px; }
.pc-dot { width: 10px; height: 10px; border-radius: 50%; opacity: .9; }
...
.feat { border-top: 1px solid var(--rf-ink); padding: 20px 8px 0; }
.feat-num { font-size: 15px; color: var(--rf-accent); margin-bottom: 16px; font-weight: 600; }
.feat-title { font-size: 19px; font-weight: 700; margin: 0 0 10px; }
.feat p { color: var(--rf-muted); font-size: 13.5px; line-height: 1.8; margin: 0; }
...
/* —— 响应 —— */
@media (max-width: 1080px) { .grid, .feat-grid { grid-template-columns: repeat(2, 1fr); } .hero-grid { grid-template-columns: 1fr; } .hero-visual { margin-top: 30px; } }
@media (max-width: 640px) { .mast-inner, .hero, .profession-section, .feature-section { padding-left: 20px; padding-right: 20px; } .grid, .feat-grid { grid-template-columns: 1fr; } }
</style>